...
# --------------------------
# Real-Time Webcam with Fog Detection + PSNR + SNR + Timing
# --------------------------
...
while True:
    ...
    if prediction <= 0.5:
        ...
        snr_output = calculate_snr(frame)
        defogged = defog_dcp(frame)
        snr_input = calculate_snr(defogged)
        psnr = calculate_psnr(frame, defogged)

        end_time = time.time()
        delay = (end_time - start_time) * 1000  # in milliseconds

        print(f"[INFO] PSNR: {psnr:.2f} dB | SNR In: {snr_input:.2f} dB | SNR Out: {snr_output:.2f} dB | Time: {delay:.2f} ms")

        cv2.putText(defogged, f"PSNR: {psnr:.2f} dB", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
        cv2.putText(defogged, f"SNR In: {snr_input:.2f} dB", (10, 90),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
        cv2.putText(defogged, f"SNR Out: {snr_output:.2f} dB", (10, 60),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
