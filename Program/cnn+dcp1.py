...
def calculate_snr(image):
    """
    Calculates the SNR of a single image using a blurred version to estimate noise.
    """
    image = image.astype(np.float32)
...
while True:
    ...
    if prediction <= 0.5:
        ...
        # Calculate PSNR for input and output
        psnr = calculate_psnr(frame, defogged)

        # Calculate SNR separately
        snr_input = calculate_snr(frame)
        ...
        print(f"[INFO] SNR  (Input / Foggy):     {snr_output:.2f} dB")
        print(f"[INFO] SNR  (Output / Defogged): {snr_input:.2f} dB")

        cv2.putText(defogged, f"PSNR: {psnr:.2f} dB", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
        cv2.putText(defogged, f"SNR: {snr_output:.2f} dB", (10, 60),
